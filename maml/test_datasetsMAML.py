...
for file in glob.glob("data/*.npz"):
	...
	if np.unique(data_y).shape[0]<=16:
		...
		if data_x.shape[0]>=20:
			f.write("& 10 ")
			results10 = np.array(compareMethodsOnSet(methods, data_x, data_y, N=10, samples=80))
		...
		if data_x.shape[0]>=60:
			f.write("& 50 ")
			results50 = np.array(compareMethodsOnSet(methods, data_x, data_y, N=50, samples=80))
